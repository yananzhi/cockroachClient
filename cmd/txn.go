...
func (ts *txnSender) Send(ctx context.Context, call client.Call) {
...
func newTxn(kv *client.KV, opts *client.TransactionOptions) *Txn {
	if opts == nil {
		opts = &defaultTxnOpts
	}
	...
	err := util.RetryWithBackoff(retryOpts, func() (util.RetryStatus, error) {
		...
		if err == nil {
			if !t.haveEndTxn && t.haveTxnWrite {
				...
				t.Prepare(client.Call{Args: etArgs, Reply: etReply})
			}
			err = t.Flush()
		}
...
// returns any errors.
func (t *Txn) Run(calls ...client.Call) error {
	if len(calls) == 0 {
		return nil
	}
...
// prepared calls.
func (t *Txn) Prepare(calls ...client.Call) {
	//	t.updateState(calls)
	//	for _, c := range calls {
...
func (t *Txn) updateState(calls []client.Call) {
	for _, c := range calls {
